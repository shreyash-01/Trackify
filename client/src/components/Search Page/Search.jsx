...
export default function Search(){
    ...
    const handleClick=()=>{
        ...
        axios.post('http://'+apiUrl+':8081/products', data)
            .then((response) => {               
    ...
    return (
        <div className="flex flex-col bg-login bg-cover h-screen bg-no-repeat">
        ...
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 flex justify-center items-center h-[40vw]">
            <motion.div initial={{ scale: 0.92, opacity:0 }} animate={{ scale: 1, opacity:1 }} transition={{ type: "spring", duration: 0.7 }} className="bg-[#0d1a31] small:h-auto small:w-[75vw] small:p-6 h-[20vw] w-[50vw] flex flex-col items-center rounded-xl shadow-2xl justify-center">
                ...
                <div className="flex justify-center">
                    <input type="text" className="mt-7 small:h-6 small:text-sm small:w-60 small:rounded-sm w-[40vw] h-[3vw] rounded-[0.5rem] text-xl outline-none pl-3 pr-2 font-lato" placeholder="Enter URL" name="url" value={searchData.url} onChange={handleChange}></input>
                </div>
                <div className="flex justify-center relative right-5 mt-7 small:mt-3">
                    <button className="bg-[#4134b6] font-open-sans small:text-[2.5vw] small:rounded-md text-[1.3vw] text-white mt-5 px-10 py-2 ml-7 rounded-[0.5rem] hover:scale-110" onClick={handleClick}>Find</button>
                </div>
            </motion.div>
        </div>
